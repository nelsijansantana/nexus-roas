...
export class ProjectsService {
  ...
  private async _ensureProjectWebhook(
    userId: string,
    project: any,
  ): Promise<string | null> {
  ...
  private async _syncProjectWebhookKV(
    webhook: any,
    userId: string,
  ): Promise<void> {
    try {
  ...
  private async _syncKV(
    project: any,
    prevCustomDomain?: string | null,
  ): Promise<void> {
    ...
    try {
      ...
      /* non-fatal — project may not have rules yet */
    }
  ...
  async resyncKV(projectId: string): Promise<void> {
    try {
      ...
      this.logger.warn(
        `[KV] resyncKV failed for project ${projectId}: ${e?.message}`,
      );
    }
  }
  ...
  private assertCanWrite(caller: JwtPayload) {
    if (caller.ownerId && caller.memberRole !== 'admin') {
      throw new ForbiddenException(
        'Seu perfil no time não permite criar ou editar projetos',
      );
    }
  }
  ...
  async findAllByUser(caller: JwtPayload) {
    ...
    if (!caller.ownerId && projects.length === 0) {
      ...
      for (const p of orphans) {
        const ownerExists = await this.prisma.users.findUnique({
          where: { id: p.userId },
        });
        if (!ownerExists) {
  ...
  async update(id: string, updateDto: UpdateProjectDto, caller: JwtPayload) {
    ...
    if (!wid) {
      ...
      updateDto.checkoutType &&
      updateDto.checkoutType !== existing.checkoutType
    ) {
      const newGateway = this._checkoutToGateway(updateDto.checkoutType);
  ...
  private _buildResponse(project: any, wid?: string | null) {
    ...
    const webhookPath = (gateway: string) => {
      if (customDomain)
        return `https://${customDomain}/collect/webhook/${gateway}`;
      if (workerUrl && wid)
        return `${workerUrl}/collect/webhook/${gateway}?wid=${wid}`;
      if (workerUrl)
        return `${workerUrl}/collect/webhook/${gateway}?pid=${pixelId}`;
      return null;
    };
    ...
    const cartpandaCheckoutScriptTag = workerPath(
      '/tracking/cartpanda-checkout.js',
    )
  ...
  async getWorkerBundle(id: string, caller: JwtPayload): Promise<Buffer> {
    ...
    const ingestApiKey = project.ingestApiKey;
    ...
    const workerRoot =
      this.config.get<string>('NEXUS_WORKER_PATH') ||
      path.resolve(process.cwd(), '..', 'nexus-worker');

    if (!fs.existsSync(workerRoot)) {
      throw new Error(
        `nexus-worker directory not found at: ${workerRoot}. Set NEXUS_WORKER_PATH env var.`,
      );
    }
