...
function deriveChannel(dto: IngestEventDto): string {
  ...
  if (dto.fbclid || src.includes('facebook') || src.includes('instagram') || src === 'fb') {
    return 'paid_social_meta';
  }
  if (dto.ttclid || src.includes('tiktok') || src === 'tt') {
    return 'paid_social_tiktok';
  }
  if (dto.gclid || (src.includes('google') && (med === 'cpc' || med === 'ppc'))) {
  ...
  if (med === 'organic' || (src.includes('google') && !med)) return 'organic_search';
...
function parseDeviceType(ua: string): string {
  ...
  if (/mobile|android.*mobile|iphone|ipod|iemobile|opera mini/.test(u)) return 'mobile';
...
export class IngestService {
  ...
  async validateKey(pixelId: string, ingestKey: string): Promise<boolean> {
    const project = await this.prisma.projects.findUnique({ where: { pixelId } });
  ...
  async ingestEvent(dto: IngestEventDto): Promise<void> {
    ...
    const nxUser     = (dto.nx_user && dto.nx_user.trim())
      ? dto.nx_user
      : (dto.lead_id && dto.lead_id.trim()) ? dto.lead_id : randomUUID();
    ...
    const customData = gateway || dto.order_id
      ? JSON.stringify({ payment_gateway: gateway, order_id: dto.order_id ?? '' })
      : '';
