...
class LocalCache {
  ...
  set(key: string, data: unknown) {
    this.store.set(key, { data, createdAt: Date.now(), refreshing: false });
  }
  markRefreshing(key: string) {
    const e = this.store.get(key);
    if (e) e.refreshing = true;
  }
  isRefreshing(key: string): boolean {
    return this.store.get(key)?.refreshing ?? false;
  }
}
...
export class AnalyticsService {
  ...
  private async _resolvePixelIds(
    caller: JwtPayload,
    projectId?: string,
  ): Promise<string[]> {
  ...
  private _timeFilter(start?: string, end?: string, col = 'event_time') {
    ...
    let sql = '';
    if (start) {
      const v = Math.floor(new Date(start).getTime() / 1000);
      if (!isNaN(v)) {
        sql += ` AND ${col} >= {startTime:UInt32}`;
        params.startTime = v;
      }
    }
    if (end) {
      const v = Math.floor(new Date(end).getTime() / 1000);
      if (!isNaN(v)) {
        sql += ` AND ${col} <= {endTime:UInt32}`;
        params.endTime = v;
      }
    }
    return { sql, params };
  ...
  private _dateFilter(start?: string, end?: string, col = 'event_date') {
    ...
    if (start) {
      ...
      if (d) {
        sql += ` AND ${col} >= {startDate:Date}`;
        params.startDate = d;
      }
    }
    if (end) {
      const d = new Date(end).toISOString().slice(0, 10);
      if (d) {
        sql += ` AND ${col} <= {endDate:Date}`;
        params.endDate = d;
      }
    }
    return { sql, params };
  ...
  async getDashboardMetrics(
    ...
    const cached =
      await this._cacheGet<ReturnType<typeof this._emptyMetrics>>(key);
    if (cached) {
      if (cached.stale) {
        void this._acquireLock(key).then((ok) => {
          ...
          void this.limiter
            .run(() => this._fetchDashboard(pixelIds, filters))
    ...
    const result = await this.limiter.run(() =>
      this._fetchDashboard(pixelIds, filters),
    );
  ...
  private async _fetchDashboard(
    ...
    const { sql: tf, params: tp } = this._timeFilter(
      filters.startDate,
      filters.endDate,
    );
    ...
    const { sql: df, params: dp } = this._dateFilter(
      filters.startDate,
      filters.endDate,
    );
    const channelQ = `
    ...
    const [totals, gateways, channels, campaigns] = await Promise.all([
      ...
      this.clickhouse.query<{
        ...
        totalRevenue: number;
        count: number;
      }>(channelQ, { pixelIds, ...dp }),
      this.clickhouse.query<{
        campaign: string;
        channel: string;
        totalRevenue: number;
        count: number;
      }>(campaignQ, { pixelIds, ...dp }),
    ]);
  ...
  async getRevenueOverTime(
    ...
    const cached =
      await this._cacheGet<{ date: string; revenue: number; sales: number }[]>(
        key,
      );
    if (cached) {
      if (cached.stale) {
        void this._acquireLock(key).then((ok) => {
          ...
          this.local.markRefreshing(key);
          void this.limiter
            .run(() => this._fetchRevenueOverTime(pixelIds, filters))
            .then((r) => this._cacheSet(key, r));
        });
      }
      return cached.data;
    ...
    const result = await this.limiter.run(() =>
      this._fetchRevenueOverTime(pixelIds, filters),
    );
    await this._cacheSet(key, result);
    return result;
  }

  private async _fetchRevenueOverTime(
    pixelIds: string[],
    filters: { startDate?: string; endDate?: string; timezone?: string },
  ) {
    const tz = filters.timezone || 'America/Sao_Paulo';
  ...
  async getChannelAttribution(
    ...
    { channel: string; revenue: number; sales: number; eventsCount: number }[]
  > {
    const pixelIds = await this._resolvePixelIds(caller, filters.projectId);
    if (!pixelIds.length) return [];
  ...
  async getSignalCoverage(
    ...
    pct_ttp: number;
    pct_gclid: number;
    capi_meta_rate: number;
    capi_tiktok_rate: number;
    capi_ga4_rate: number;
  }> {
    const pixelIds = await this._resolvePixelIds(caller, filters.projectId);
