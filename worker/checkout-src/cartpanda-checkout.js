...
// ── Identity ──────────────────────────────────────────────────────────────────
// Multi-source: cookie nx_user > cookie nx_lid > localStorage > URL src param
var nxUser = (function () {
...
var shopifyCartToken = getCookie('shopify_cart_token') || '';

// ── UTMs + click IDs from checkout URL ───────────────────────────────────────
...
// Click IDs — read from URL params and persist in session cookies so they
// survive CartPanda's multi-step checkout redirects.
...
// ── Extract ecommerce data from a CartPanda DataLayer entry ───────────────────
// CartPanda stores items in eventModel.items (not ecommerce.items).
// Value is in eventModel.value or eventModel.ecomm_totalvalue.
function extractEcomm(entry) {
  var src      = entry.eventModel || entry.ecommerce || {};
  var items    = src.items || entry.items || [];
  var value    = parseFloat(src.value || src.ecomm_totalvalue || entry.value || 0) || 0;
  var currency = src.currency || entry.currency || 'BRL';
  var numItems = 0;
  var contentIds = [], contents = [], ga4Items = [];

  items.forEach(function (it) {
    var id    = String(it.item_id || it.product_id_sp || it.variant_id || '');
    ...
    if (id) {
      ...
      ga4Items.push({ item_id: id, item_name: it.item_name || '', price: price, quantity: qty });
    }
  });
...
function fire(nexusEvent, ecomm, userData) {
  ...
  var payload = {
    ...
    event_id:           eid,
    nx_user:            nxUser,
    shopify_cart_token: shopifyCartToken || undefined,
...
// ── DataLayer observer ────────────────────────────────────────────────────────
// CartPanda pushes begin_checkout synchronously via GTM before our async script
// loads. We must process the existing DataLayer AND watch for future pushes.
var firedEvents = {};

function processEntry(entry) {
  if (!entry || typeof entry !== 'object') return;
  var ev = (entry.event || '');
  if (!ev || firedEvents[ev]) return;

  var nexusEvent = null;
  if (ev === 'begin_checkout')           nexusEvent = 'InitiateCheckout';
  else if (ev === 'begin_checkout_info') nexusEvent = 'AddShippingInfo';
  else if (ev === 'add_payment_info')    nexusEvent = 'AddPaymentInfo';

  if (!nexusEvent) return;
  firedEvents[ev] = true;
  fire(nexusEvent, extractEcomm(entry));
}
...
// Fallback: se begin_checkout não apareceu no DataLayer em 2s, dispara sem dados
setTimeout(function () {
  if (!firedEvents['begin_checkout']) fire('InitiateCheckout');
}, 2000);
